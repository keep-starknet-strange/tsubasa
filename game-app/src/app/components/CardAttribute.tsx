...
function getColorClassnames(
  ...
  if (bonus) {
    return {
      backgroundClassname: "bg-mint-green",
      textClassname: "text-black",
    };
  } else if (hurt) {
    return {
      backgroundClassname: "bg-cherry-red",
    ...
    switch (team) {
      case "blue":
        return {
          backgroundClassname: pending ? "bg-pine-green" : "bg-turquoise",
          textClassname: pending ? "text-white" : "text-black",
        };

      case "yellow":
        return {
          backgroundClassname: pending ? "bg-mustard-seed" : "bg-sunny-yellow",
          textClassname: pending ? "text-white" : "text-black",
        };

      case "red":
        return {
          backgroundClassname: pending ? "bg-cocoa" : "bg-peach-pink",
          textClassname: pending ? "text-white" : "text-black",
        };
    }
  }
...
const CardAttribute: FC<CardAttributeProps> = (props) => {
  ...
  const { backgroundClassname, textClassname } = getColorClassnames(
    team,
    pending,
    bonus,
    hurt
  ...
  return (
    <div
      className={`${backgroundClassname} ${textClassname} inline-flex flex-col items-center justify-center rounded p-1 pt-2 font-new-airport font-medium shadow`}
    >
      <div className={`font-extrabold opacity-50`}>{value}</div>
